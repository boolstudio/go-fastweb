...
func callMethod(data reflect.Value, name string) (result reflect.Value, found bool) {
	...
	if len(parts) > 1 {
		param = reflect.NewValue(parts[1])
	}
	for {
		typ := data.Type()
...
func call(v reflect.Value, method reflect.Method, param reflect.Value) reflect.Value {
